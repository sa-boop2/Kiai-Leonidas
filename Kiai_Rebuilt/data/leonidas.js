 ...
 * Namespaced for Dual-Mode Integration in Kiai Premium
 */
...
const LEONIDAS_STORAGE_KEY = "leonidas_stoic_protocol_v4";
const LEONIDAS_THEME_KEY = "leonidas_theme_pref";
...
class ModernRoadmapApp {
  ...
  loadState() {
    const saved = localStorage.getItem(LEONIDAS_STORAGE_KEY);
    if (saved) {
      try {
    ...
    const savedTheme = localStorage.getItem(LEONIDAS_THEME_KEY);
    if (savedTheme) {
  ...
  saveState() {
    localStorage.setItem(LEONIDAS_STORAGE_KEY, JSON.stringify({
  ...
  resetBaselineData() {
    if (confirm("Reset all custom measurements and actuals back to original Leonidas protocol baseline?")) {
  ...
  toggleTheme() {
    ...
    localStorage.setItem(LEONIDAS_THEME_KEY, this.state.theme);
    this.applyTheme(this.state.theme);
  }

  applyTheme(theme) {
    const leoShell = document.getElementById("app-leonidas");
    if (leoShell) leoShell.setAttribute("data-theme", theme);
    const icon = document.getElementById("leo_sidebarThemeIcon");
    const text = document.getElementById("leo_sidebarThemeText");
    if (icon && text) {
  ...
  setupEventListeners() {
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        document.querySelectorAll("#app-leonidas .modal-overlay.active").forEach(m => m.classList.remove("active"));
      }
    });
  }
  ...
  navigate(viewId) {
    ...
    document.querySelectorAll(".leo-view-section").forEach(sec => {
      sec.classList.toggle("active", sec.id === `leo-view-${viewId}`);
    });

    document.querySelectorAll(".leo-nav-btn").forEach(btn => {
      const isActive = btn.id === `leo-nav-${viewId}`;
      btn.classList.toggle("active", isActive);
      btn.setAttribute("aria-selected", isActive);
    });

    document.querySelectorAll(".leo-bottom-nav-item").forEach(btn => {
      btn.classList.toggle("active", btn.id === `leo-bot-${viewId}`);
    });
  ...
  renderAll() {
    ...
    const rIcon = document.getElementById("rankIcon");
    if (rIcon) rIcon.textContent = rankInfo.currentRankObj.icon;
    const rTitle = document.getElementById("rankTitle");
    if (rTitle) rTitle.textContent = `Rank ${rankInfo.currentRankObj.rank} — ${rankInfo.currentRankObj.title}`;
    const rDesc = document.getElementById("rankDesc");
    if (rDesc) rDesc.textContent = rankInfo.currentRankObj.desc;
    
    const rNext = document.getElementById("rankNextLabel");
    if (rNext) {
      if (rankInfo.currentRankObj.rank === "VII") {
        rNext.textContent = `🏆 Final Master Goal Achieved`;
      } else {
        rNext.textContent = `Next: Rank ${rankInfo.nextRankObj.rank} — ${rankInfo.nextRankObj.title} (${rankInfo.nextRankObj.minLBM} kg LBM)`;
      }
    }

    const rPctVal = document.getElementById("rankPercentVal");
    if (rPctVal) rPctVal.textContent = `${rankInfo.progressPct}% Progression`;
    const rProgBar = document.getElementById("rankProgressBar");
    if (rProgBar) rProgBar.style.width = `${rankInfo.progressPct}%`;
    ...
    const cTitle = document.getElementById("centralPhaseTitle");
    if (cTitle) cTitle.textContent = p.name;
    const cDesc = document.getElementById("centralPhaseDesc");
    if (cDesc) cDesc.textContent = p.desc;
    const cGoal = document.getElementById("centralPhaseGoal");
    if (cGoal) cGoal.textContent = `${p.targetWeight.toFixed(1)} kg @ ${p.targetBF.toFixed(1)}% BF`;
    const cLBM = document.getElementById("centralPhaseLBM");
    if (cLBM) cLBM.textContent = `Target Lean Body Mass: ${p.targetLBM.toFixed(1)} kg`;

    const actualValEl = document.getElementById("centralPhaseActual");
    const actualLbmEl = document.getElementById("centralActualLBM");
    if (actualValEl && actualLbmEl) {
    ...
    if (rateEl) {
      ...
      rateEl.style.color = p.type === "CUT" ? "var(--leo-crimson)" : "var(--leo-emerald)";
    }
    const cCals = document.getElementById("centralPhaseCals");
    if (cCals) cCals.textContent = p.cals;
    const cWks = document.getElementById("centralPhaseWks");
    if (cWks) cWks.textContent = `${p.weeks} (${p.period})`;
  ...
  renderRoadmapTable() {
    ...
    tbody.innerHTML = this.state.phases.map(p => {
      ...
      let actualDisplay = `<span style="color: var(--leo-text-tertiary);">—</span>`;
      if (p.actualWeight !== null) {
        actualDisplay = `<strong style="color: var(--leo-emerald);">${p.actualWeight.toFixed(1)} kg @ ${p.actualBF.toFixed(1)}%</strong>`;
      }

      return `
        <tr class="${isCurrent ? "active-phase" : ""}">
          <td><strong style="${isCurrent ? "color: var(--leo-gold);" : ""}">${p.name}</strong></td>
          ...
          <td style="font-weight: 700; color: ${p.type === "CUT" ? "var(--leo-crimson)" : "var(--leo-emerald)"};">${p.rate}</td>
          <td>
            <button class="leo-btn ${isCurrent ? "leo-btn-primary" : "leo-btn-glass"}" style="padding: 6px 14px; font-size: 0.8rem;" onclick="Leonidas.openEditModal(${p.id})">
              ✏️ Log / ${statusText}
            </button>
          </td>
        </tr>
      ...
      return `
        <div class="rank-card ${isUnlocked ? "unlocked" : ""} ${isCurrent ? "current" : ""}">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <span style="font-family: var(--leo-font-header); font-size: 1.4rem; font-weight: 800; color: ${isCurrent ? "var(--leo-gold)" : "var(--leo-text-tertiary)"};">Rank ${r.rank}</span>
          </div>
          <div>
            <h3 style="font-family: var(--leo-font-header); font-size: 1.2rem; font-weight: 800; color: var(--leo-text-primary); margin-bottom: 4px;">${r.title}</h3>
            <div style="font-size: 0.8rem; font-weight: 700; color: var(--leo-gold); text-transform: uppercase; margin-bottom: 8px;">Target: ${r.minLBM > 0 ? `${r.minLBM} kg LBM` : "Transformation Baseline"}</div>
            <p style="color: var(--leo-text-secondary); font-size: 0.9rem;">${r.desc}</p>
          </div>
          <div style="margin-top: auto; padding-top: 12px; border-top: 1px solid var(--leo-border-glass); font-size: 0.78rem; font-weight: 800; color: ${isCurrent ? "var(--leo-gold)" : isUnlocked ? "var(--leo-emerald)" : "var(--leo-text-disabled)"}; text-transform: uppercase;">
  ...
  renderPhaseSelectorModal() {
    ...
    container.innerHTML = this.state.phases.map(p => {
      const isCurrent = p.id === this.state.activePhaseId;
      return `
        <div onclick="Leonidas.setActivePhaseQuick(${p.id})" style="display: flex; justify-content: space-between; align-items: center; padding: var(--leo-space-md); background: ${isCurrent ? "rgba(245, 179, 1, 0.15)" : "var(--leo-bg-tertiary)"}; border: 1px solid ${isCurrent ? "var(--leo-gold)" : "var(--leo-border-glass)"}; border-radius: var(--leo-radius-md); cursor: pointer; transition: all 0.2s;">
          <div>
            <div style="font-weight: 800; color: ${isCurrent ? "var(--leo-gold)" : "var(--leo-text-primary)"}; font-size: 1.05rem;">${p.name}</div>
            <div style="font-size: 0.85rem; color: var(--leo-text-tertiary);">${p.period} (${p.weeks})</div>
          </div>
  ...
  evaluateQuiz() {
    ...
    if (!resBox || !resTitle || !resText) return;

    resBox.style.display = "block";

    if (bulkChecked >= 3) {
      resTitle.textContent = "🛑 CEASE BULKING — SWITCH TO CUT PHASE";
      resTitle.style.color = "var(--leo-crimson)";
      resBox.style.borderColor = "var(--leo-crimson)";
      ...
      resTitle.style.color = "var(--leo-emerald)";
      resBox.style.borderColor = "var(--leo-emerald)";
      ...
      resTitle.style.color = "var(--leo-gold)";
      resBox.style.borderColor = "var(--leo-gold)";
...
window.Leonidas = new ModernRoadmapApp();
